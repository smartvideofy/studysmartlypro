  ...
  return useMutation({
    mutationFn: async (contentType: ContentType) => {
      ...
      if (error) {
        ...
        if (errorMessage.includes("quota") || errorMessage.includes("OpenAI") || error.context?.status === 402) {
          throw new Error("OpenAI API quota exceeded. Please check your API key usage.");
        }
        throw error;
      }
      return data;
